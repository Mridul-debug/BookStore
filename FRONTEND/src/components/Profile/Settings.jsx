...
const Settings = () => {
  ...
  const submitAddress=async()=>{
    const response=await axios.put("https://bookheaven-04ur.onrender.com/api/v1/update-address",Value,{headers})
    alert(response.data.message)
  }
  useEffect(()=>{
    const fetch=async()=>{
      const response=await axios.get(
        "https://bookheaven-04ur.onrender.com/api/v1/get-user-information",
        {headers}
      )
        ...
        <div className='h-full p-0 md:p-4 text-zinc-100'>
          ...
          <div className='flex gap-12'>
            ...
            <div className=''>
              ...
              <p className='p-2 rounded bg-zinc-800 mt-3 font-semibold'>
                {ProfileData.email}
              </p>
            </div>
          </div>
